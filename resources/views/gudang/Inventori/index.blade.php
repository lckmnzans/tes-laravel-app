    ...
    <body x-data="main" class="relative overflow-x-hidden font-nunito text-sm font-normal antialiased" :class="[ $store.app.sidebar ? 'toggle-sidebar' : '', $store.app.theme === 'dark' || $store.app.isDarkMode ?  'dark' : '', $store.app.menu, $store.app.layout,$store.app.rtlClass]">
        ...
        <div class="main-container min-h-screen text-black dark:text-white-dark" :class="[$store.app.navbar]">
                    ...
                    <div class="h-full bg-white dark:bg-[#0e1726]">
                        ...
                        <ul class="perfect-scrollbar relative h-[calc(100vh-80px)] space-y-0.5 overflow-y-auto overflow-x-hidden p-4 py-0 font-semibold" x-data="{ activeDropdown: 'dashboard' }">
                            ...
                            <li class="nav-item">
                                ...
                                <li class="nav-item">
                                    <a href="{{ route('gudang.inventori.index') }}" class="group">
                                        <div class="flex items-center">
                                            <svg class="shrink-0 group-hover:!text-primary" width="20" height="20" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path opacity="0.5" d="M3 10C3 6.22876 3 4.34315 4.17157 3.17157C5.34315 2 7.22876 2 11 2H13C16.7712 2 18.6569 2 19.8284 3.17157C21 4.34315 21 6.22876 21 10V14C21 17.7712 21 19.6569 19.8284 20.8284C18.6569 22 16.7712 22 13 22H11C7.22876 22 5.34315 22 4.17157 20.8284C3 19.6569 3 17.7712 3 14V10Z" fill="currentColor"></path>
                                            ...
                                            <span class="text-black ltr:pl-3 rtl:pr-3 dark:text-[#506690] dark:group-hover:text-white-dark">Manajemen Inventori</span>
                                        </div>
                                    </a>
                                </li>    
                                <li class="nav-item">
                                        <a href="apps-todolist.html" class="group">
                                            <div class="flex items-center">
                                                ...
                                                <span class="text-black ltr:pl-3 rtl:pr-3 dark:text-[#506690] dark:group-hover:text-white-dark">Notifikasi Stok Minimum</span>
                                            </div>
                                        </a>
                                    </li>
                                    <h2 class="-mx-4 mb-1 flex items-center bg-white-light/30 py-3 px-7 font-extrabold uppercase dark:bg-dark dark:bg-opacity-[0.08]">
                                        ...
                                        <span>Pengeluaran Penerimaan Bahan Baku</span>
                                    </h2>    
                                    <li class="nav-item">
            ...
            <div class="-me-2 flex items-center sm:hidden">
                ...
                <div class="animate__animated p-6" :class="[$store.app.animation]">
                    ...
                    <div x-data="inventoris">
                        <div class="flex flex-wrap items-center justify-between gap-4">
                            <h2 class="text-xl">Data Inventori</h2>
                            <div class="flex w-full flex-col gap-4 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
                                <div class="flex gap-3">
                                    <div>
                                        <a href="{{route('gudang.inventori.create') }}" class="btn btn-primary" @click="editUser">
                                ...
                                <div class="relative">
                                    <input type="text" placeholder="Cari Produk" class="peer form-input py-2 ltr:pr-11 rtl:pl-11" x-model="searchUser" @keyup="searchProducts">
                            ...
                            <template x-if="displayType === 'list'">
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            @foreach($inventoris as $key => $inventori)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ $inventori->nama_bahan }}</td>
                                                <td>{{ $inventori->jenis_bahan }}</td>
                                                <td>{{ $inventori->tanggal_penerimaan }}</td>
                                                <td>{{ $inventori->supplier->nama_perusahaan ?? 'Tidak ada data' }}</td>
                                                <td>{{ $inventori->jumlah }}</td>
                                                <td>{{ $inventori->jumlah_stok_tersedia }}</td>
                                                <td>{{ $inventori->safety_stock }}</td>
                                                <td>Rp {{ number_format($inventor-i>harga_per_unit, 0, ',', '.') }}</td>
                                                <td>Rp {{ number_format($inventori->harga_per_unit * $inventori->jumlah, 0, ',', '.') }}</td>
                                                <td>{{ $inventori->lokasi_stok }}</td>
                                                <td>{{ $inventori->no_penerimaan }}</td>
                                                <td>{{ $inventori->catatan ?? '-' }}</td>
                                                <td class="text-center">
                                                    <div class="flex items-center justify-center gap-2">
                                                        <a href="{{ route('gudang.inventori.edit', $inventori->id) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                                                        <form action="{{ route('gudang.inventori.destroy', $inventori->id) }}" method="POST" style="display:inline;">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Anda yakin ingin menghapus bahan baku ini?')">Delete</button>
                                                        </form>
                                                    </div>
                                                </td>
                                            </tr>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                ...
                //produts
                Alpine.data('inventoris', () => ({
                    displayType: 'list', // tipe tampilan: daftar atau grid
                    addinventoriModal: false, // kontrol tampilan modal
                    searchinventori: '', // teks pencarian
                    bahaBakuList: @json($inventoriBakuList), // inisialisasi data bahan baku dari Laravel
                    filteredInventoriList: [],
                    params: {
                        id: null,
                        nama_bahan: '',
                        jenis_bahan: '',
                    ...
                    init() {
                        this.filteredInventoriList = this.nventorList; // daftar bahan baku diawali dengan semua data
                    },

                    searchInventoriItems() {
                        // Filter daftar bahan baku berdasarkan input pencarian
                        this.filteredInventoriList = this.InventoriList.filter((bahan) =>
                            bahan.nama_bahan.toLowerCase().includes(this.searchInventori.toLowerCase())
                        );
                    },

                    editInventori(bahan) {
                        // Buka modal dengan data bahan baku, jika ada
                        if (bahan) {
                            this.params = { ...bahan };
                        } else {
                            this.params = {
                        ...
                        this.addInventoriModal = true;
                    },

                    async saveInventori() {
                        // Cek apakah semua kolom wajib telah terisi
                        if (!this.params.nama_bahan || !this.params.jenis_bahan || !this.params.tanggal_penerimaan || !this.params.supplier || !this.params.harga) {
                            return this.showMessage('Semua kolom wajib diisi.', 'error');
                        }

                        // Hitung total harga secara otomatis
                        this.params.total_harga = this.params.harga * this.params.jumlah;

                        try {
                            let response;
                            const url = this.params.id ? `/gudang/inventori/index/${this.params.id}` : '/gudang/inventori/index';
                    ...
                    async deleteInventori(Inventori) {
                        // Hapus data inventori
                        try {
                            const response = await fetch(`/gudang/inventori/index/${inventori.id}`, {
                                method: 'DELETE',
                                headers: {
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                    'Content-Type': 'application/json'
                            ...
                            if (data.success) {
                                ...
                                this.showMessage('Gagal menghapus bahan baku.', 'error');
                            }
                        } catch (error) {
                            this.showMessage('Terjadi kesalahan. Coba lagi.', 'error');
                        }
